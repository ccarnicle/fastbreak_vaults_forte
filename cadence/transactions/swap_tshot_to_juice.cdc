...
transaction(amount: UFix64) {
    ...
    let amountInTshot: UFix64

    /// Temporary FLOW vault withdrawn from signer
    let tshotVault: @{FungibleToken.Vault}
    ...
    prepare(acct: auth(BorrowValue, Storage) &Account) {
        ...
        self.amountInTshot = amount

        // Withdraw FLOW from the signer's FlowToken vault
        let tshotVaultRef = acct.storage
        ...
        self.tshotVault <- tshotVaultRef.withdraw(amount: self.amountInTshot)
        ...
        self.path = [ "A.05b67ba314000b2d.TSHOT", "A.1654653399040a61.FlowToken", "A.d6f80565193ad727.stFlowToken", "A.9db94c9564243ba7.aiSportsJuice" ]
    }
    ...
    execute {
        ...
        let juiceVault <- swapper.swap(quote: nil, inVault: <-self.tshotVault)
