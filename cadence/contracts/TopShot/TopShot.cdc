/*
  For local emulator use: keep up to date with https://github.com/dapperlabs/nba-smart-contracts/blob/master/contracts/TopShot.cdc
...
access(all) contract TopShot: NonFungibleToken {
    ...
    access(all) view fun Network() : String { return "emulator" } //fake address for emulator

    // The address to which royalties should be deposited
    access(all) view fun RoyaltyAddress() : Address { return 0x123456 } //fake address for emulator
